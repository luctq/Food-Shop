			...
			<c:forEach var="item" items="${ productsPaginate }" varStatus="loop">
				<div class="row-fluid">
					<ul class="thumbnails">
						<li class="span4">
							<div class="thumbnail">
								<a href="product_details.html" class="overlay"></a> <a
									...
									VIEW</a> <a href="<c:url value="/chi-tiet-san-pham/${ item.id_products }"/>"><img
						...
						<c:if
							test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == productsPaingate.size() }">
					</ul>
				</div>
				<c:if test="${ (loop.index + 1) < productsPaginnate.size() }">
					<div class="row-fluid">
						<ul class="thumbnails">
				</c:if>
		</c:if>
